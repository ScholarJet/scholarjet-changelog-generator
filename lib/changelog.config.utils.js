...
function _updateJiraTicket(jiraTicket, repoUrl) {
  let updatedTicket = {
    ...jiraTicket
  };

  updatedTicket.tickets = updatedTicket.tickets.map(ticket => {
    const updatedCommits = ticket.commits.map(commit => updatePRInCommit(commit, repoUrl));
    return {
      ...ticket,
...
function _updateTicketSubTasks(tickets, subTasks) {
  ...
  for (let i = 0; i < subTasks.length; i++) {
    updated = _updateStoriesAndBugs(updated, subTasks[i]);
  }
...
function processTicketsForTemplateUsage(tickets, repoUrl) {
  return _.chain(_getTicketsWithSubTasks(_simplifyIssueTypeInformation(tickets)))
    ...
    .value().map(ticket => _updateJiraTicket(ticket, repoUrl));
}
